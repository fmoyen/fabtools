...
{
set -x
time svmon -Ss > $svskfile
vmstat -v > $vmstatvfile
time svmon -G > $svgfile
time svmon -S > $svsfile
time svmon -m -Pn > $svpfile
time svmon -Sl > $svslfile   #MATT
set +x
}
...
svsfile_sids=$1
svmon_sids_mem=0
grep -v Vsid $svsfile_sids|awk '{print substr($0,53,2),substr($0,56,6)}'|while read sz pg; do
        case "$sz" in
...
echo "==========================================================================="
echo "Node: `/usr/bin/hostname`           Date: `/usr/bin/date`"
echo "==========================================================================="

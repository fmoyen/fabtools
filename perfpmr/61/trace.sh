...
do_trace()
{
  ...
  echo "$trace $PROPAGATE $PROCESS_LIST $THREAD_LIST $PURR -j 106,10C,10E,112,113,134,139,465,46D,46E,606,607,608,609 $ltype -n $CPUS -d -L $LOGBUF -T $KERNBUF -ao $TRAWL"
  $trace $PROPAGATE $PROCESS_LIST $THREAD_LIST $PURR -j 106,10C,10E,112,113,134,139,465,46D,46E,606,607,608,609 $ltype -n $CPUS -d -L $LOGBUF -T $KERNBUF -ao $TRAWL
  else

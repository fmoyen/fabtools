...
show_usage()
{
  ...
  echo "\nperfpmr.sh: Usage: \n"
  echo "  perfpmr.sh [-hPQDIgfnpscEHA][-r][-F file][-x file][-d sec] [-W program [-w sec]][-o "outputdirs"][-Z][-z paxfile] monitor_seconds"
...
validate_fs()
{
        /usr/bin/df $PWD | /usr/bin/tail -1 | /usr/bin/awk '{print $1,$NF}'  | read lv fs
        if [ "$fs" != "" ]; then
             /usr/sbin/mount | fgrep -w "$fs" | awk '{print $3,$4,$7}' | read fstype1 fstype2 options
             if echo $options | egrep 'cio|dio' >/dev/null; then
             ...
             if echo $fstype1 $fstype2 | egrep 'nfs|dfs|autofs' >/dev/null; then
...
if [ "$1" != "getargs" ]; then
	...
	if whence hpmcount >/dev/null; then
		$PERFPMRDIR/hpmcount.sh  $@ 2>/dev/null| $TEEOUT
	fi
	return
fi
...
END {
	...
	if ( TPROF_DFLAG == 1 )
		tprof_D_flag= "-D"
	printf("%s perfpmr_tprof %s %s %s %s %s\n", space, tprof_search_path, tprof_E_flag, tprof_D_flag, tprof_samp_freq,tprof_time_secs);
} '  default_time=$default_time_tprof TPROF_EFLAG=$TPROF_EFLAG  TPROF_DFLAG=$TPROF_DFLAG $PERFPMRCFG
}  
...
#  fi
#
#  instfix -ik IZ22917 >/dev/null 2>&1
#  if [ $? -ne 0 ]
#  then
...
{
	echo " " 1>&2
	echo "(C) COPYRIGHT International Business Machines Corp., 2000,2001,2002,2003,2004-2008" 1>&2
	echo " " 1>&2
	sleep 1
}
...
PERFPMRTIME=1245099011   # number of seconds since EPOCH for version of perfpmr
...
while getopts AXEHW:w:Zz:o:QIPDd:F:x:gcfnpsr flag 2>/dev/null; do
        case $flag in
	       A)     AMS_SYSTEM=1;;
	       E)     TPROF_EFLAG=1;;
	       X)     PERFXTRA_FLAG=1;;

...
np=`echo "mempool *"| kdb |awk 'BEGIN { mp=0 }{ if (mp == 1) { if ($1 != "(0)>") print $0 } if ($1=="VMP") { mp = 1 } }'|wc -l`
nf=`echo "frameset *"| kdb |awk 'BEGIN { mp=0 }{ if (mp == 1) { if ($1 != "(0)>") print $0 } if ($1=="FRS") { mp = 1 } }'|wc -l`

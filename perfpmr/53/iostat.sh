...
#iostat -p 2>&1|grep "No tapes found" >/dev/null && iostat_tape_exists=1
iostat  -p -T  $INTERVAL $COUNT > iostat.p &
aioconfig=`lsattr  -E -laio0 -a autoconfig |awk '{print $2}'`
if [ "$aioconfig" = defined ]; then

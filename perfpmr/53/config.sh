...
do_lsmcode()
{
	do_timestamp "lsmcode -A"
	lsmcode -A >> microcode_levels.out
}
...
do_tunables_FL()
{
echo "\n\nVMO SETTINGS  (vmo -a)" >> $RESTNGOUT
echo     "-------------------------\n"  >> $RESTNGOUT
if [ -f /usr/sbin/vmo ]; then
     /usr/sbin/vmo -a >> $RESTNGOUT
     echo "\n\nIOO SETTINGS  (ioo -a)" >> $RESTNGOUT
     echo     "----------------------------\n"  >> $RESTNGOUT
     /usr/sbin/ioo -a   >> $RESTNGOUT 2>&1
     echo "\n\nSCHEDO SETTINGS   (schedo -a)" >> $RESTNGOUT
     echo     "--------------------------------\n"  >> $RESTNGOUT
     /usr/sbin/schedo -a >> $RESTNGOUT 2>&1
     echo "\n\nNETWORK OPTIONS:  (no -a)" >> $RESTNGOUT
     echo     "-------------------------\n"  >> $RESTNGOUT
     /usr/sbin/no -a >>  $RESTNGOUT

     echo "\n\nVMO SETTINGS  (vmo -L)" >> $RESTNGOUT
     echo     "-------------------------\n"  >> $RESTNGOUT
     /usr/sbin/vmo -L >> $RESTNGOUT
     echo "\n\nIOO SETTINGS  (ioo -L)" >> $RESTNGOUT
     echo     "----------------------------\n"  >> $RESTNGOUT
     /usr/sbin/ioo -L   >> $RESTNGOUT 2>&1
     echo "\n\nSCHEDO SETTINGS   (schedo)" >> $RESTNGOUT
     echo     "--------------------------------\n"  >> $RESTNGOUT
     /usr/sbin/schedo -L >> $RESTNGOUT 2>&1
     echo "\n\nNETWORK OPTIONS:  (no -L)" >> $RESTNGOUT
     echo     "-------------------------\n"  >> $RESTNGOUT
     /usr/sbin/no -L >>  $RESTNGOUT
else
...
do_aioo()
{
# collect output of aio
if [ -f /usr/sbin/aioo ]; then
	echo "\n\nAIOO SETTINGS   (aio)" >> $CFGOUT
	echo     "--------------------------------\n"  >> $CFGOUT
     	/usr/sbin/aioo -a >> $CFGOUT
fi
}
...
{
# =====================  VMPOOL STATISTICS ===============================
echo "\n\nMEMPOOL STATS (getvmpool.sh  )" >> $CFGOUT
echo     "--------------------------------------------\n"  >> $CFGOUT
do_timestamp "getvmpool.sh"
...
echo "kmbucket -s" | /usr/sbin/kdb  | egrep 'alloc|wall|kmemin' >> $CFGOUT
}
...
do_lvmo
if [ -d /usr/ios ]; then
	if lslpp -cl bos.perf.perfstat | grep 5.3.0.60 >/dev/null; then
		echo "This VIO server needs to have bos.perf.perfstat updated"
	else
		do_ras
	fi
else
